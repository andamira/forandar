...
class XTerm {
  ...
  ///
  /// mode 1002: CHECK button movement reporting, reports movement when a button is pressed.
  ///               reset: ?
  ///
  /// mode 1003: CHECK all movement reporting, reports all movements.
  ///               reset: ?
  ///
  /// mode 1004: CHECK FocusIn/FocusOut can be combined with any of the mouse events
  ///            since it uses a different protocol. When set, it causes xterm to send
  ///            CSI I when the terminal gains focus, and CSI O when it loses focus.
  ///               reset: ?
  ///
  /// mode 1005: CHECK Extended mouse mode enables UTF-8 encoding for C x and C y
  ///            under all tracking modes, expanding the maximum encodable position
  ///            from 223 to 2015. For positions less than 95, the resulting output
  ///            is identical under both modes. Under extended mouse mode, positions
  ///            greater than 95 generate "extra" bytes which will confuse applications
  ///            which do not treat their input as a UTF-8 stream. Likewise, C b will
  ///            be UTF-8 encoded, to reduce confusion with wheel mouse events.
  ///               reset: ?
  ///
  /// mode 1034: Interpret "meta" key, sets eighth bit.
  ///            (enables the eightBitInput resource).
  ///     reset: Don't interpret "meta" key. (This disables
  ...
  static void mouseOn({hilite: false})  => writeXterm(1001 + (hilite ? 1 : 0));

  /// Disables sending Mouse X & Y on button press and release.
  static void mouseOff({hilite: false}) => writeXterm(1001 + (hilite ? 1 : 0), reset: true);
